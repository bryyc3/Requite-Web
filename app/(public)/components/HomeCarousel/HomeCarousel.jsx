                ...
                {slides.map((slide, i) => (
                    <div key ={i} 
                         className="homeSlide absolute inset-0 opacity-0 translate-x-10 
                          ...
                          data-index ={i}>
                        <h1 className="font-bold text-[clamp(1rem,2cqi,3em)] leading-none">{slide.heading}</h1>
                        <h2 className="font-extralight text-[clamp(.75rem,1.5cqi,2rem)]">{slide.subheading}</h2>
                    </div>
                ))}
